...
```
    ...
    ## # A tibble: 5 x 7
    ##   week  day_id day    minute count  hour weekend
    ##   <fct>  <dbl> <fct>   <dbl> <dbl> <int> <fct>  
    ## 1 1          1 Friday      1  88.4     0 No     
    ## 2 1          1 Friday      2  82.2     0 No     
    ## 3 1          1 Friday      3  64.4     0 No     
    ## 4 1          1 Friday      4  70.0     0 No     
    ## 5 1          1 Friday      5  75.0     0 No
...
total = 
  accel %>% 
    group_by(week, weekend, day) %>% 
    summarize(total = sum(count)) %>% 
    arrange(weekend, day) %>% 
    select(week, weekend, day, total) %>% 
    pivot_wider(names_from = day,                            
                values_from = total
              ) %>%        
 knitr::kable(caption = "Total activities for each day")
```

**Answer** From the table above, we can see that the patient’s activty
level is higher during weekdays than on weekends. Starting from the 4th
