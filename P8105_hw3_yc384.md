...
```
    ...
    ##   <int> <chr> <chr>                  <fct>         <dbl>
    ## 1  2010 AL    AL - Jefferson County  Poor            5.5
    ## 2  2010 AL    AL - Mobile County     Poor            6.4
    ## 3  2010 AL    AL - Tuscaloosa County Poor            4.2
...
```
    ...
    ##    year state location              response  data_value
    ##   <int> <chr> <chr>                 <fct>          <dbl>
    ## 1  2002 WV    WV - Kanawha County   Excellent       16.5
    ## 2  2002 WI    WI - Milwaukee County Excellent       21.1
    ## 3  2002 WY    WY - Laramie County   Excellent       25.5
...
**Answer:**From above results, we can see that in 2002, there were 36
...
**Answer:**From above results, we can see that in 2010, there were 45
states that were observed at 7 or more locations. Much more than year
2002.
...
<img src="P8105_hw3_yc384_files/figure-gfm/unnamed-chunk-8-1.png" width="100%" />
...
``` r
# made a plot table for year 2006
ny = 
   brfss %>% 
    select(year, response, state, data_value) %>% 
  ...
  facet_grid(. ~ year) +
  labs(
    title = "NY Locations Observed in 2006 and 2010",
      x = "Response",
      y = "Data_value",
